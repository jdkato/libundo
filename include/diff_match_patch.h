 ...
 * and some tweaks for std::string by leutloff@sundancer.oche.de (Christian
 Leutloff)
 *
...
class diff_match_patch {
  ...
  */
  class Diff {
   public:
     ...
     */
    Diff(Operation _operation, const string_t &_text)
    ...
    string_t toString() const {
      ...
      for (typename string_t::iterator i = prettyText.begin();
           i != prettyText.end(); ++i)
        if (traits::to_wchar(*i) == L'\n') *i = traits::from_wchar(L'\u00b6');
      return traits::cs(L"Diff(") + strOperation(operation) +
             traits::cs(L",\"") + prettyText + traits::cs(L"\")");
    }
  ...
  */
  class Patch {
   public:
    ...
    bool isNull() const {
      return start1 == 0 && start2 == 0 && length1 == 0 && length2 == 0 &&
             diffs.size() == 0;
    }
    ...
    string_t toString() const {
      ...
      if (length1 == 0) {
        ...
        coords1 = to_string(start1 + 1) + traits::from_wchar(L',') +
                  to_string(length1);
      }
      if (length2 == 0) {
        ...
        coords2 = to_string(start2 + 1) + traits::from_wchar(L',') +
                  to_string(length2);
      }
      string_t text(traits::cs(L"@@ -") + coords1 + traits::cs(L" +") +
                    coords2 + traits::cs(L" @@\n"));
      // Escape the body of the patch with %xx notation.
      for (typename Diffs::const_iterator cur_diff = diffs.begin();
           cur_diff != diffs.end(); ++cur_diff) {
 ...
 public:
   ...
   */
  Diffs diff_main(const string_t &text1, const string_t &text2,
                  bool checklines = true) const {
   ...
   */
 private:
  static void diff_main(const string_t &text1, const string_t &text2,
                        bool checklines, clock_t deadline, Diffs &diffs) {
    diffs.clear();
    ...
    if (text1 == text2) {
      ...
      textChopped1 =
          textChopped1.substr(0, textChopped1.length() - commonlength);
      textChopped2 =
          textChopped2.substr(0, textChopped2.length() - commonlength);
   ...
   */
 private:
  static void diff_compute(string_t text1, string_t text2, bool checklines,
                           clock_t deadline, Diffs &diffs) {
    if (text1.empty()) {
   ...
   */
 private:
  static void diff_lineMode(string_t text1, string_t text2, clock_t deadline,
                            Diffs &diffs) {
    ...
    for (typename Diffs::iterator cur_diff = diffs.begin();
         cur_diff != diffs.end(); ++cur_diff) {
      switch ((*cur_diff).operation) {
        case INSERT:
          count_insert++;
   ...
   */
 protected:
  static Diffs diff_bisect(const string_t &text1, const string_t &text2,
                           clock_t deadline) {
    Diffs diffs;
    diff_bisect(text1, text2, deadline, diffs);
    return diffs;
 ...
 private:
  static void diff_bisect(const string_t &text1, const string_t &text2,
                          clock_t deadline, Diffs &diffs) {
   ...
   */
 private:
  static void diff_bisectSplit(const string_t &text1, const string_t &text2,
                               int x, int y, clock_t deadline, Diffs &diffs) {
   ...
   *     encoded text2 and the List of pointers to unique strings.  The zeroth
   * element
 ...
 protected:
    ...
    bool operator<(const LinePtr &p) const {
      return this->second < p.second
                 ...
                 : this->second > p.second
                       ? false
                       : string_t::traits_type::compare(this->first, p.first,
                                                        this->second) < 0;
    }
  };
  ...
  static void diff_linesToChars(string_t &text1, string_t &text2,
                                Lines &lineArray) {
    ...
    for (typename std::map<LinePtr, size_t>::const_iterator i =
             lineHash.begin();
         i != lineHash.end(); ++i)
  ...
  /**
   * Split a text into a list of pointers to strings.  Reduce the texts to a
   * string of
   * hashes where each Unicode character represents one line.
 ...
 protected:
  static int diff_commonOverlap(const string_t &text1, const string_t &text2) {
    ...
    while (true) {
      ...
      length += found;
      if (found == 0 ||
          right(text1_trunc, length) == text2_trunc.substr(0, length)) {
        best = length;
        length++;
      }
    }
 ...
 protected:
  struct HalfMatchResult {
    ...
    void swap(HalfMatchResult &hm) {
      text1_a.swap(hm.text1_a), text1_b.swap(hm.text1_b),
          text2_a.swap(hm.text2_a), text2_b.swap(hm.text2_b),
          mid_common.swap(hm.mid_common);
    }
  };

  static bool diff_halfMatch(const string_t &text1, const string_t &text2,
                             HalfMatchResult &hm) {
    ...
    bool hm1 =
        diff_halfMatchI(longtext, shorttext, (longtext.length() + 3) / 4, res1);
    // Check again based on the third quarter.
    bool hm2 =
        diff_halfMatchI(longtext, shorttext, (longtext.length() + 1) / 2, res2);
    if (!hm1 && !hm2) {
      return false;
    } else if (!hm2) {
      hm.swap(res1);
      ...
      hm.swap(res1.mid_common.length() > res2.mid_common.length() ? res1
                                                                  : res2);
    }
 ...
 private:
  static bool diff_halfMatchI(const string_t &longtext,
                              const string_t &shorttext, int i,
 ...
 public:
  static void diff_cleanupSemantic(Diffs &diffs) {
    ...
    if ((cur_diff = diffs.begin()) != diffs.end()) {
      for (typename Diffs::iterator prev_diff = cur_diff;
 ...
 public:
  static void diff_cleanupSemanticLossless(Diffs &diffs) {
    ...
    for (typename Diffs::iterator next_diff = cur_diff;
         ++next_diff != diffs.end();
   ...
   */
 private:
  static int diff_cleanupSemanticScore(const string_t &one,
                                       const string_t &two) {
    ...
    if (lineBreak1) {
      typename string_t::const_reverse_iterator p1 = one.rbegin(),
                                                p2 = one.rend();
      if (traits::to_wchar(*p1) == L'\n' && ++p1 != p2) {
        if (traits::to_wchar(*p1) == L'\r') ++p1;
 ...
 public:
  void diff_cleanupEfficiency(Diffs &diffs) const {
    ...
    for (typename Diffs::iterator cur_diff = diffs.begin();
      ...
      if ((*cur_diff).operation == EQUAL) {
        // Equality found.
        if ((int)(*cur_diff).text.length() < Diff_EditCost &&
            (post_ins || post_del)) {
          // Candidate found.
          equalities.push_back(cur_diff);
          pre_ins = post_ins;
              ...
              if (commonlength != 0) {
                if (cur_diff != diffs.begin()) {
                  ...
                  if ((*--prev_diff).operation != EQUAL) {
                    throw string_t(traits::cs(
                        L"Previous diff should have been an equality."));
                  }
                  (*prev_diff).text += text_insert.substr(0, commonlength);
                } else {
                  diffs.insert(cur_diff, Diff(EQUAL, text_insert.substr(
                                                         0, commonlength)));
                }
      ...
      for (typename Diffs::iterator next_diff = cur_diff;
        ...
        if ((*prev_diff).operation == EQUAL &&
          ...
          if ((*cur_diff).text.size() >= (*prev_diff).text.size() &&
              (*cur_diff).text.compare(
                  (*cur_diff).text.size() - (*prev_diff).text.size(),
                  (*prev_diff).text.size(), (*prev_diff).text) == 0) {
            // Shift the edit over the previous equality.
            (*cur_diff).text =
                (*prev_diff).text +
                (*cur_diff).text.substr(
                    0, (*cur_diff).text.length() - (*prev_diff).text.length());
            ...
            (*cur_diff).text =
                safeMid((*cur_diff).text, (*next_diff).text.length()) +
                (*next_diff).text;
  ...
  static string_t diff_toDelta(const Diffs &diffs) {
    ...
    for (typename Diffs::const_iterator cur_diff = diffs.begin();
      ...
      switch ((*cur_diff).operation) {
        ...
        case DELETE:
          text += traits::from_wchar(L'-') +
                  to_string((*cur_diff).text.length()) +
                  traits::from_wchar(L'\t');
          break;
        case EQUAL:
          text += traits::from_wchar(L'=') +
    ...
    for (typename string_t::const_pointer token = delta.c_str();
      ...
      switch (traits::to_wchar(*token)) {
        ...
        case L'=': {
          ...
          n = to_int(param);
          if (n < 0) {
            throw string_t(traits::cs(L"Negative number in diff_fromDelta: ") +
                           param);
          }
          string_t text;
        ...
        default:
          throw string_t(string_t(traits::cs(
                             L"Invalid diff operation in diff_fromDelta: ")) +
                         *token);
      }
    }
 ...
 public:
   ...
   */
 protected:
  int match_bitap(const string_t &text, const string_t &pattern,
                  int loc) const {
    ...
    for (int d = 0; d < (int)pattern.length(); d++) {
      ...
      int finish =
          std::min(loc + bin_mid, (int)text.length()) + pattern.length();
 ...
 private:
   ...
   */
 protected:
  static void match_alphabet(const string_t &pattern,
                             std::map<char_t, int> &s) {
    // There is no need to initialize map values, since they are
 ...
 public:
   ...
   */
 public:
  Patches patch_make(const string_t &text1, const string_t & /*text2*/,
 ...
 public:
  ...
  void patch_apply(const Patches &patches, const string_t &sourceText,
    ...
    Patches patchesCopy(
        patches);  // Default copy constructor will do it just fine
      ...
      if (start_loc == -1) {
        ...
        if (text1 == text2) {
          ...
          if ((int)text1.length() > Match_MaxBits &&
            ...
            diff_cleanupSemanticLossless(diffs);
            int index1 = 0;
            for (typename Diffs::const_iterator cur_diff =
                     (*cur_patch).diffs.begin();
    ...
    res.first = safeMid(text, nullPadding.length(),
 ...
 public:
  string_t patch_addPadding(Patches &patches) const {
    ...
    for (typename Patches::iterator cur_patch = patches.begin();
  ...
  void patch_splitMax(Patches &patches) const {
    ...
    for (typename Patches::iterator cur_patch = patches.begin();
      ...
      while (!bigpatch.diffs.empty()) {
        ...
        precontext =
            safeMid(diff_text2(patch.diffs),
  ...
  Patches patch_fromText(const string_t &textline) const {
    ...
    if (!textline.empty()) {
      ...
      while (text - textline.c_str() < (int)textline.length()) {
        if ((text_len =
                 next_token(textline, traits::from_wchar(L'\n'), text)) == 0) {
          ++text;
          continue;
        }

        // A replacement for the regexp "^@@ -(\\d+),?(\\d*) \\+(\\d+),?(\\d*)
        // @@$" exact match
        ...
        for (text += text_len + 1;
             text - textline.c_str() < (int)textline.length();
             text += text_len + 1) {
          if ((text_len =
                   next_token(textline, traits::from_wchar(L'\n'), text)) == 0)
            continue;
 ...
 private:
  ...
  static typename string_t::size_type next_token(
      const string_t &str, char_t delim, typename string_t::const_pointer off) {
  ...
  static void percent_decode(string_t &str) {
    ...
    for (typename string_t::const_pointer s1 = str.c_str(),
                                          end = s1 + str.size();
         s1 != end; ++s1, ++s2)
...
struct diff_match_patch_utf32_from_utf16 {
  ...
  static iterator to_utf32(iterator i, iterator end, utf32_t &u) {
    ...
    if (UTF16_SURROGATE_MIN <= u && u <= UTF16_HIGH_SURROGATE_MAX && i != end)
      u = (u << 10) + *i++ -
          UTF16_SURROGATE_OFFSET;  // Assume it is a UTF-16 surrogate pair
    return i;
  }
  template <class iterator>
  static iterator from_utf32(utf32_t u, iterator o) {
    if (u > 0xFFFF) {  // Encode code points that do not fit in char_t as UTF-16
                       // surrogate pairs
      *o++ = static_cast<char_t>((u >> 10) + UTF16_SURROGATE_MIN -
                                 (0x10000 >> 10));
...
#include <cwctype>
template <>
struct diff_match_patch_traits<wchar_t>
  ...
  static int to_int(const wchar_t *s) {
    return static_cast<int>(std::wcstol(s, NULL, 10));
  }
...
struct diff_match_patch_traits<char> : diff_match_patch_utf32_direct<char> {
  ...
  static std::string cs(const wchar_t *s) {
